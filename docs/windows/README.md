...
- ~~GitHub Actions `windows-latest` CI matrix~~ — **now live.** `checks.yml` runs an advisory `windows-checks` job (typecheck + `test:main`) on `windows-latest`; it is not yet a required status check.
