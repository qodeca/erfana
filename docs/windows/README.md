...
4. **Check open issues**: `gh issue list --repo qodeca/erfana --state open`. There is no `windows` label on the public repository yet — run `gh label list --repo qodeca/erfana` first, and only add `--label windows` once one exists.
