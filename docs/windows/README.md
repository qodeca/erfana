...
2. **Branch strategy**: Phases 0–2 shipped in v0.9.3 on `develop`. **Phase 4 (local Whisper) is code-complete on `feature/windows-phase-4-whisper` awaiting PR merge for 0.9.4** — 13 commits, adds self-hosted whisper.cpp binary workflow + full client-side trust chain. Phase 3 + Phases 5–6 use their own `feature/windows-phase-<N>-*` branches off `develop`. The historical `windows` integration branch was deleted after the 2026-04-22 merge.
...
**One-line current state (2026-04-23):** Phases 0, 1, 2 shipped in **v0.9.3** (merged from `windows` → `develop` on 2026-04-22). **Phase 4 (local Whisper, [#165](https://github.com/qodeca/erfana/issues/165)) code-complete on `feature/windows-phase-4-whisper` for 0.9.4** — 13 commits, unreleased. Phase 3 (screenshots, [#164](https://github.com/qodeca/erfana/issues/164)) + Phases 5–6 ([#166](https://github.com/qodeca/erfana/issues/166) / [#167](https://github.com/qodeca/erfana/issues/167)) unstarted. Deferred items D1–D12 → [#168](https://github.com/qodeca/erfana/issues/168). Dependabot triage → [#169](https://github.com/qodeca/erfana/issues/169).
