...
| `src/renderer/src/components/Panels/TerminalPanel.flickering.test.tsx` | 5 s timeout on "should enforce integer dimensions during resize" (Windows local, 2026-04-22). Not reproduced on later runs. | 2026-04-22 | 🟡 under observation | — | If reproduces on `windows-latest` CI (Phase 6 #167), file follow-up. Likely another wall-clock assertion needing fake timers. |
| `src/renderer/src/utils/panelUtils.test.ts` (`waitForTerminalReady` custom interval) | 151 ms wall-clock for a 100 ms timing test (Windows local, 2026-04-22). Borderline. | 2026-04-22 | 🟡 under observation | — | Likely same pattern as SettingsOverlay focus — worker pre-emption on tight timer. Fix proactively if it reproduces. |
