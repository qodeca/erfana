...
### Open Windows-tagged issues (refreshed 2026-08-07)
...
3. **Check open Windows issues:** `gh issue list --repo qodeca/erfana --state open`. There is no `windows` label on the public repository yet — run `gh label list --repo qodeca/erfana` before adding `--label windows`, and filter by hand until one exists.
...
- `src/main/services/whisper-assets.ts` — Phase 4 landed; canonical `classifyPlatform()` + per-platform `reason` strings live here. Edit when changing the cross-platform Whisper support matrix; `SettingsOverlay.tsx:81-87` mirrors this logic by comment (kept in lockstep manually).
