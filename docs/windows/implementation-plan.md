...
*Last updated 2026-09-15, anchored on v0.20.0. The 2026-09-15 Windows validation of everything since v0.19.0 ([report](../handoffs/2026-09-15-windows-post-0.19.0-report.md)) found no product defect: every failure traced to a test with a macOS-shaped assumption, fixed in [#128](https://github.com/qodeca/erfana/pull/128)–[#130](https://github.com/qodeca/erfana/pull/130), and 51 tests Windows had never run now run there. Open items from that report: a dependency advisory gap (§7), the electron-builder bump verified on CI only, an intermittent drag-freeze flake, and five hands-on checks. The prior v0.19.0 anchor note follows. The 2026-09-03 Windows verification ([handoff](../handoffs/2026-09-03-windows-release-verification.md)) ran 32 of the 34 manual HTML-preview checks and all nine automated gates on a Windows 11 host; it found two blockers – the host-approval band freezing on "Saving…" and an evicted preview tab never waking – plus a per-open/close OS-handle leak, and the fixes landed on `fix/windows-preview-blockers` (see [`CHANGELOG.md`](../CHANGELOG.md) § 0.19.0). A macOS-host run of the same suite on 2026-09-04 found the still-frame half of the evicted-tab fix failing there for a different reason — the retry that produces the only non-empty capture was abandoned when the tab was switched away inside its 250 ms window — fixed on `fix/preview-still-frame-capture`; the three `*-darwin.png` visual baselines, left stale by the 2026-09-04 capture-clip change, were regenerated in the same branch. Four checks were recorded as unrun on the day; a 2026-09-04 re-audit retested two of them – a project on a second drive letter and a UNC path – and both pass. **Two remain not run**: display scaling at 125% and 150% (needs a system-wide setting on a single-monitor host), and the Zoom In keystroke (no automation on this host can deliver it, though its route is now covered in CI by `src/main/menu.test.ts`). Both are detailed in [`known-flakes.md`](known-flakes.md) § "Untested on the 2026-09-03 Windows verification". The prior v0.18.0 anchor note follows (v0.18.0 adds the HTML preview and image-export/refresh features — both cross-platform, with no Windows-phase scope change; the prior v0.17.2 anchor note is retained below. Two v0.17.2 entries are Windows-relevant: `DirectoryWatcherService` unit tests were made deterministic under load on Windows hosts, and the 2026-08-12 Windows e2e flake observations were recorded in [`known-flakes.md`](known-flakes.md). The headline v0.17.2 fix — the large-project black-window crash plus the renderer and main-process error-containment layers ([#60](https://github.com/qodeca/erfana/issues/60)) — is cross-platform and applies on Windows unchanged). Release history relevant to Windows: **Phase 3 (screenshots) shipped in [v0.12.0](https://github.com/qodeca/erfana/releases/tag/v0.12.0)** (tag 2026-06-04) via [PR #208](https://github.com/qodeca/erfana/pull/208) (merge `9b2f91a`). **Phase 6 (polish, [#167](https://github.com/qodeca/erfana/issues/167)) partially shipped in [v0.13.0](https://github.com/qodeca/erfana/releases/tag/v0.13.0)** — items 1, 4, 7 (advisory CI), the single-bridge renderer platform detection, PII log redaction, and D5/D7 done and tagged; items 2 (CameraService) and 3 (ProjectLockService) verified working with no code change; D4 promoted to its own ticket ([#220](https://github.com/qodeca/erfana/issues/220)); items 5/6 + the forward-looking parity items remain (now tracked as [#221](https://github.com/qodeca/erfana/issues/221)–[#224](https://github.com/qodeca/erfana/issues/224)). **[v0.14.0](https://github.com/qodeca/erfana/releases/tag/v0.14.0) (tag 2026-06-06) added Windows-quality polish outside the formal Phase-6 issue scope**: bundled Cascadia Mono font for cross-platform terminal parity; project-tree header now shows the folder name on Windows instead of the full `C:\…` path; git-status worker prefers native `git` to fix CRLF false-positives and detects repo transitions; the OS window title shows project + version on both platforms; Claude Code context bar transcript selection is floored by `claude` process start time so a fresh session no longer mis-reports a prior session's percentage ([#225](https://github.com/qodeca/erfana/issues/225)). **[v0.15.0](https://github.com/qodeca/erfana/releases/tag/v0.15.0) (tag 2026-06-09) hardened multi-instance reliability on both platforms** — `ProjectLockService` lock files are HMAC-signed against same-user forgery, a heartbeat reclaims stale locks from a crashed window after 30 s, sleep/wake correctly refreshes held locks, symlinked lock paths are refused (junction-redirect / CVE-2025-68146 class), and the **Windows** process-liveness check (`isProcessRunning`) now fails closed on unknown errors rather than assuming a process is dead (closes the Phase-6 item 3 verification scope for `ProjectLockService`). **[v0.15.1](https://github.com/qodeca/erfana/releases/tag/v0.15.1) (tag 2026-06-10) fixed the Windows-specific parent-folder git-status dot** ([#237](https://github.com/qodeca/erfana/issues/237)) — `gitStatus.logic`'s parent-path lookup now recognises both `/` and `\`, restoring the colored folder indicator on Windows. Cross-platform companion: Project Tree git badges now auto-refresh after editing an existing file via a chokidar `change` listener in `DirectoryWatcherService` ([#241](https://github.com/qodeca/erfana/issues/241)). **Windows support for the Claude Code context status bar** ([PR #245](https://github.com/qodeca/erfana/pull/245), closes [#217](https://github.com/qodeca/erfana/issues/217)) landed on `develop` after v0.15.1 and first reached users in [v0.16.3](https://github.com/qodeca/erfana/releases/tag/v0.16.3) — `WinClaudeProcessDetector` (single static PowerShell `Win32_Process` BFS, fail-closed, liveness-cached, win32-path resolution), platform-branched `encodeCwd`, a shared `AbstractClaudeProcessDetector` base, turn-aware transcript selection (skips metadata-only sidecars), mid-session model-switch window tracking, and a shutdown chokidar timer-race fix that was the root cause of the Windows e2e EBUSY teardown timeout. Single-panel detection verified on a Windows host; two-panel UAT pending (see [technical-debt.md §12](../technical-debt.md)). **v0.16.0, v0.16.1 and v0.16.2 were never published** — v0.16.0 was superseded before tagging and v0.16.1/v0.16.2 were burned by Windows signing failures (Azure secret mismatch, then a legacy-RC2/3DES PFX rejected by OpenSSL 3.x). [v0.16.3](https://github.com/qodeca/erfana/releases/tag/v0.16.3) (tag object dated 2026-06-17; GitHub release published 2026-06-18) was the first public open-source release, shipping the macOS arm64 DMG + Windows NSIS installer pair; **[v0.17.0](https://github.com/qodeca/erfana/releases/tag/v0.17.0) (tag 2026-08-08) shipped** the macOS Screen Recording grant-and-relaunch flow (macOS-only; other platforms keep the existing toast fallback), the camera hot-plug/mirroring fixes, and the Claude Code context-meter model-id fix (both cross-platform, applying on Windows unchanged). **[v0.17.1](https://github.com/qodeca/erfana/releases/tag/v0.17.1) (tag 2026-08-10) shipped** the DOCX-export SSRF strip and 250 MB import cap ([#57](https://github.com/qodeca/erfana/issues/57)), the electron-builder packaging-guard extensions to `extraFiles`/`extraResources` ([#55](https://github.com/qodeca/erfana/issues/55), [#43](https://github.com/qodeca/erfana/issues/43)), and the Claude Code context-meter post-compaction freeze fix ([#47](https://github.com/qodeca/erfana/issues/47)) — all cross-platform or build-side and applying on Windows unchanged. **[v0.17.2](https://github.com/qodeca/erfana/releases/tag/v0.17.2) (tag 2026-08-12) shipped** the large-project black-window fix plus the renderer and main-process error-containment layers ([#60](https://github.com/qodeca/erfana/issues/60)) — cross-platform — together with the Windows `DirectoryWatcherService` test-determinism fix. **[v0.18.0](https://github.com/qodeca/erfana/releases/tag/v0.18.0) (tag 2026-08-25) shipped** the sandboxed live HTML preview ([#74](https://github.com/qodeca/erfana/issues/74)), PNG/PDF/clipboard image export ([#73](https://github.com/qodeca/erfana/issues/73)) and the image-viewer disk-refresh fix ([#70](https://github.com/qodeca/erfana/issues/70)) — all cross-platform, applying on Windows unchanged. **[v0.19.0](https://github.com/qodeca/erfana/releases/tag/v0.19.0) (tag 2026-09-04) shipped** the per-tab HTML previews, in-preview link handling, page zoom and the preview toolbar ([#74](https://github.com/qodeca/erfana/issues/74) follow-ups), including the two blockers the 2026-09-03 Windows verification found. **[v0.20.0](https://github.com/qodeca/erfana/releases/tag/v0.20.0) (tag 2026-09-16) shipped** same-project frames, same-tab links with Back and Forward, Open in default browser and keyboard entry into the page ([#124](https://github.com/qodeca/erfana/issues/124)) — cross-platform, with the win32-specific paths validated on a Windows host on 2026-09-15 (frames, 8.3 short names, alternate data streams; the open items are listed at the top of this snapshot).*
...
**Phase 3 (screenshot parity) shipped in [v0.12.0](https://github.com/qodeca/erfana/releases/tag/v0.12.0)** (merged to `develop` 2026-06-03 via [PR #208](https://github.com/qodeca/erfana/pull/208), merge `9b2f91a`; tagged 2026-06-04) — UAT verified on a Windows host 2026-06-04. **Phase 4 (local Whisper) merged to `develop` on 2026-04-23 ahead of Phase 3** — the order was driven by [#165](https://github.com/qodeca/erfana/issues/165) scope becoming urgent once step-zero verification surfaced the ggml-org macOS gap. Every phase that has been started is therefore released; Windows work resumes on **Phase 5 (NSIS UX, [#166](https://github.com/qodeca/erfana/issues/166))** — the next unstarted phase, since auto-updater and Windows signing already shipped via #174 in v0.9.5.

### Open Windows work items (issue numbers are pre-migration provenance)

Verified 2026-08-07 against the public repository: **none of the issue numbers below exist there.**
`qodeca/erfana` renumbered from #1 at the 2026-06 open-source migration (it topped out at #42 on
2026-08-07 and at #120 on 2026-09-05), so #166/#167/#168/#177 resolve to nothing — they name the
item, they do not track it. Pre-migration numbers at or below #120 are worse than dead: they now
resolve to unrelated public issues, so treat any bare number in this folder as provenance only. The
work itself is still open, and nothing has been re-filed on the public tracker yet. Run
`gh issue list --repo qodeca/erfana --label windows --state open` for what is actually tracked today.
...
- [#167](https://github.com/qodeca/erfana/issues/167) — Phase 6: polish, Windows CI guard, visual baselines (#158 reference dropped; D6 owned by Phase 5). Partially shipped in v0.13.0 — item 6 (visual baselines) landed 2026-09-04, so item 5 is what remains
...
Already closed before the migration (never in the open list): #158 (closed 2026-04-22), #163 (deferred-decision, closed 2026-04-21).
Closure queued in the 2026-06-03 alignment pass: #164 (Phase 3 shipped v0.12.0), #165 (Phase 4 shipped v0.9.4 — closure posts the trust-chain summary as a comment), #169 (Dependabot — `npm audit` re-run before closing to confirm clean per CHANGELOG v0.11.2; falls back to body-trim if not). None of these issues survived the 2026-06 repository migration, so the closures were never posted.

---

## Feature status on Windows today

Honest per-feature assessment of what an end user running an NSIS install of v0.20.0 gets today. Phases 0–4 are shipped; Phase 5 is narrowed to NSIS UX and Phase 6 is partially shipped.
...
| **Screenshot capture** (screen / window / area) (#164) | Phase 3, shipped v0.12.0. `ScreenshotService` dispatches to `DesktopCapturerScreenshotCapturer` on win32 (Electron `desktopCapturer` + per-display area-select overlay + in-app `WindowPickerDialog`) |
| **Local Whisper transcription** (offline, model download) (#165) | Phase 4, shipped v0.9.4. `classifyPlatform()` accepts `win32-x64`; self-hosted signed binaries with the 4-layer trust chain. Windows ARM64 and pre-SSE4.2 CPUs are still rejected by design |
| **Signed NSIS installer** (#174) | Shipped v0.9.5. Azure Artifact Signing via X.509 certificate auth; SmartScreen reputation ramp tracked in [#177](https://github.com/qodeca/erfana/issues/177) |

### ❌ Not available on Windows

Three of the four rows that used to sit here have shipped and moved to the ✅ table above: **screenshot capture** (Phase 3, v0.12.0 — `ScreenshotService` dispatches to `DesktopCapturerScreenshotCapturer` on win32), **local Whisper transcription** (Phase 4, v0.9.4 — `classifyPlatform()` in `whisper-assets.ts` accepts `win32-x64`; the `WhisperModelManager.getArchSuffix()` throw described here no longer exists anywhere in `src/`), and **signed installs** (#174, v0.9.5 — Azure Artifact Signing).

| Feature | Why | Tracked |
|---|---|---|
| **Auto-updater** | Not broken — **deliberately absent on both platforms.** `electron-builder.yml` sets `publish: null`; in-app auto-update is an explicit non-goal of [#174](https://github.com/qodeca/erfana/issues/174), recorded as an architectural decision in [`gap-analysis.md`](gap-analysis.md) row B6. Users update by downloading the next release. | Closed by decision, not by code |
| **SmartScreen reputation** | Installers *are* signed (Azure Artifact Signing, v0.9.5+), but a new certificate accumulates SmartScreen reputation over time; early downloads may still see an "unrecognized app" prompt | [#177](https://github.com/qodeca/erfana/issues/177) |
...
| ~~**Camera photo capture**~~ | ✅ Verified working on a Windows host — Phase 6 item 2 (2026-06-05, pure WebRTC + `os.tmpdir()`, no code change) and again during the Phase 3 UAT on 2026-06-04 | Resolved |
| ~~**Stale project-lock recovery**~~ | ✅ Verified Phase 6 item 3 (2026-06-05) and hardened in v0.15.0 — HMAC-signed lock files, 30 s heartbeat reclaim of stale locks, symlinked lock paths refused, Windows `isProcessRunning` fails closed | Resolved |
| **Packaged OCR** (tessdata resolution in NSIS install) | Unverified in packaged build vs. dev | Phase 6 |
| **`test:cov` clean exit on Windows** | ~~Tests pass; vitest v8 coverage aggregator hits ENOENT race on NTFS~~ — **superseded 2026-09-04**: the aggregator race is not the cause. Every test passes; two **per-file coverage floors** (`scripts/fuses.js`, `src/main/utils/tarArchive.ts`) are missed because both suites skip their symlink cases on win32 | [#158](https://github.com/qodeca/erfana/issues/158) closed 2026-04-22 without code change. Real reason and status in [`known-flakes.md` § `npm run test:cov` cannot pass on a Windows host](known-flakes.md#npm-run-testcov-cannot-pass-on-a-windows-host) |
| **Duplicate-instance focus attention** | macOS bounces dock via `app.dock?.bounce`; Windows steals focus via `setAlwaysOnTop` without taskbar flash | Phase 6 polish (untracked GH issue) — `focusWindow.ts:43-63` should call `BrowserWindow.flashFrame(true)` |
| **Recent projects in jumplist / dock menu** | Not wired; `app.addRecentDocument` never called on project open. Both OSes silently miss recent projects | Phase 6 polish (untracked GH issue) — `main/index.ts:303-334` |
| **Windows menubar About / Help entry** | macOS app menu has About via `role: 'about'`; Windows menubar has File/Edit/View/Window only; users cannot see app version in-product | Phase 6 polish (untracked GH issue) — `menu.ts:21-97` |
| **File deletion** | Both platforms permanently delete via `fs.rm`; no Trash / Recycle Bin via `shell.trashItem` | Phase 6 polish (untracked GH issue) — `FileService.ts:318-352` |
...
1. **cmd.exe bootstrap** (fixes B3) — `TerminalService.ts` win32 branch
   ...
   - Bare `cd` (no args) prints the working directory — the cmd.exe analog of `pwd`. The marker triggers the same handshake at `TerminalService.ts:292-332`.
...
4. **cwd validation deny-list** (BLOCKER #2 from review) — `validateWindowsCwd` rejects cwds containing any of `["&|^<>\r\n]` before constructing the bootstrap. `createTerminal` returns `{ error }` (was `null` before v0.19.0), logs an error, and emits an `'error'` event. **Hard contract**: callers must surface the error to the user. Since v0.19.0 a cwd over 260 chars is refused the same way, with a sentence naming the Win32 limit. **(Phase-2 UAT update)**: `(` and `)` were removed from the deny-list so paths under `C:\Program Files (x86)\…` are accepted; parens are cmd metacharacters only outside quotes and are literal inside `cd /d "<cwd>"`.
...
1. **[#160](https://github.com/qodeca/erfana/issues/160) — Git allowlist** (P1, smallest) — `workers/git-status.worker.ts:28`
...
4. **[#163](https://github.com/qodeca/erfana/issues/163) — `isWindowsLongPath`** (P2, decision-deferred) — `PlatformConfig.ts:205`
   - Decision recorded inline at `PlatformConfig.ts:188` (comment block) with promotion criteria: re-activate as P1 if any real >260-char path victim surfaces in UAT, OR any Phase 3+ feature produces >200-char default paths.
...
All four sub-issues + umbrella closed 2026-04-21. Phase 2 **shipped in v0.9.3** on 2026-04-22 (merge `c1e085d`, release `0b593a1`). Phase 3 (screenshots, [#164](https://github.com/qodeca/erfana/issues/164)) **shipped in [v0.12.0](https://github.com/qodeca/erfana/releases/tag/v0.12.0)** (merged to `develop` 2026-06-03 via [PR #208](https://github.com/qodeca/erfana/pull/208), `9b2f91a`; tagged 2026-06-04). Next phase up: Phase 5 (NSIS UX, [#166](https://github.com/qodeca/erfana/issues/166)).

---

## Phase 3 — Screenshot parity — ✅ Shipped in [v0.12.0](https://github.com/qodeca/erfana/releases/tag/v0.12.0) (tag 2026-06-04)

**Why:** Feature parity with the macOS modes (full screen, window, area) on Windows.

**What shipped in v0.12.0** (merged to `develop` 2026-06-03, `9b2f91a`, via [PR #208](https://github.com/qodeca/erfana/pull/208); tagged 2026-06-04)**:**
...
**Verification (2026-06-04):** all three modes (screen / window / area) captured successfully on a Windows host; camera capture also exercised; clean app quit. Path quoting verified in cmd. The darwin-host regression check named in the PR test plan was never recorded as executed; it is closed as superseded rather than passed — `MacScreenshotCapturer` carries the pre-#164 `screencapture` flow unchanged, and it has shipped in every macOS build from v0.12.0 through v0.16.3 (five subsequent releases) with no reported regression.
...
- `.github/workflows/whisper-binaries.yml` — 4-job workflow (`validate-inputs` on ubuntu-latest, `build-macos` on macos-14, `build-windows` on windows-latest, `publish-release` on ubuntu-latest) gated on the `production-signing` GitHub Environment.
...
**Tracking:** [#167](https://github.com/qodeca/erfana/issues/167) — **partially shipped in [v0.13.0](https://github.com/qodeca/erfana/releases/tag/v0.13.0)** (tag 2026-06-05): items 1, 4 and 7, plus the renderer platform-detection bridge, PII log redaction and D5/D7. Items 5 and 6 remain unreleased; item 3's `ProjectLockService` scope was closed later, in v0.15.0. Per-item status as of the v0.13.0 cut is below.
...
3. ✅ **Verify `ProjectLockService`** (fixes M10) — **verified working on Windows as-is 2026-06-05**, no code change *at the time*. The empirical test on Win11 / Node 24 confirmed `process.kill(pid, 0)` throws `ESRCH` for dead PIDs (disproving a prior no-op claim), so `isProcessAlive()` / stale-lock recovery behaved correctly for the dead-PID case. **v0.15.0 superseded the "no code change needed" conclusion**: `isProcessRunning` now fails closed on *unknown* errors instead of assuming the process is dead — the 2026-06-05 check covered `ESRCH` only, not the ambiguous-error path — alongside HMAC-signed lock files, a 30 s heartbeat reclaim of stale locks and refusal of symlinked lock paths.
...
6. ✅ **Generate `-win32.png` visual baselines** (fixes m6) — **done 2026-09-04.** All six baselines are generated and committed, and `npx playwright test --project=visual` passes 6/6 on a Windows 11 host. The page-level captures (welcome, editor, terminal) were clipped to the requested 1280x800 first, so those three no longer record host window chrome. **Residual**: `settings-overlay` and `confirm-dialog` are overlay *elements* that fill the window, so their capture is the element bounds and the committed baselines still carry this host's 1384x861 content size — a Windows host with a different content size will diff on those two until they are re-captured or given an explicit size.
7. ✅ **Windows CI guard** (deferred from Phase 0) — **done 2026-06-05 as an advisory (non-required) check.** Added a `windows-checks` job to `.github/workflows/checks.yml` running `typecheck + design-sync check + test:main` on `windows-latest`. **NOT a required status check yet** — promote to required once it proves stable. Wiring visual regression into this job is queued behind re-enabling the E2E workflow (`e2e.yml` is disabled), not behind baselines: item 6 landed the `-win32.png` set on 2026-09-04.
...
| ~~Phase 6 visual baselines `-win32.png`~~ — **done 2026-09-04** (all six committed, `--project=visual` 6/6 on a Windows host) | ✅ | ❌ | — |
...
3. **Check open Windows issues:** `gh issue list --repo qodeca/erfana --label windows --state open`. The `windows` label exists on the public repository (verified 2026-08-07 via `gh label list`) and is the session-start filter used across this folder.
...
- `src/main/services/whisper-assets.ts` — Phase 4 landed; canonical `classifyPlatform()` + per-platform `reason` strings live here. Edit when changing the cross-platform Whisper support matrix; `SettingsOverlay.tsx:81-86` mirrors this logic by comment (kept in lockstep manually).
...
- `window.api.utils.getPlatform()` preload bridge (`getPlatform` in `src/preload/index.ts`) — already exists; renderer usage is consolidated on it via `src/renderer/src/utils/platform.ts`
- `PlatformConfig.isWindowsLongPath()` (`PlatformConfig.ts:205`) — exists as dead code, activate it in #163
- `ProjectService.ts:61-63` — case-insensitive path comparison pattern; reuse in any new path equality check
- `file-handlers.ts:555-557` — Windows system-path blocklist pattern; reuse shape for reserved-name guard in #161
...
- Existing `markerDetector` handshake in `TerminalService.ts:292-332` — reused by the cmd.exe bootstrap (Phase 1 landed)
...
- `src/main/index.ts:313` — macOS dock menu (`app.dock.setMenu`). Windows has no equivalent (jumplist serves the same purpose; see next entry).
- `src/main/index.ts:318` — Windows jumplist (`app.setJumpList`). macOS uses dock menu instead.
- `src/main/utils/focusWindow.ts:43-63` — three-way focus strategy. macOS calls `app.dock?.bounce`; Windows uses the `setAlwaysOnTop` trick; Linux falls back to plain `focus()`. **Forward-looking gap:** Windows should also call `BrowserWindow.flashFrame(true)` (tracked in §"Degraded" below).
- `src/main/utils/spawnNewInstance.ts:84,114` — three-way spawn strategy for new instances (`darwin` branch at `:84`, `win32` at `:114`).
- `src/main/menu.ts:21-97` — macOS app menu (`role: 'about'`, Hide/HideOthers/Unhide); Windows menubar has File/Edit/View/Window only. **Forward-looking gap:** Windows menubar should expose About/Help (tracked in §"Degraded" below).
- `src/main/services/LocalWhisperService.ts:835-838` — POSIX vs Win32 CPU-unsupported exit-code sets. POSIX set is macOS-only by accident-of-support (Linux is rejected upstream at `whisper-assets.ts:199-202`); any Linux enablement must validate the set against Linux SIGILL exit codes.
...
11. **Auto-update (Phase 5):** nothing to verify — `electron-builder.yml` sets `publish: null` and in-app auto-update is an explicit non-goal (#174). Confirm the app makes no update request on launch.
