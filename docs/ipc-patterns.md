...
- System schemas — OS-integration actions with no payload: `system:openScreenRecordingSettings` (opens the macOS Screen Recording privacy pane) and `system:relaunchApp` (macOS only applies a fresh Screen Recording grant to a newly-launched process). Channel constants in `src/shared/ipc/system-channels.ts`; both handlers are sender-gated main-side in `src/main/ipc/system-handlers.ts`
