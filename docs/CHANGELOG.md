...
- **Phase-2 UAT hardening (2026-04-21 session)** — surfaced and closed during dev-build UAT on the `windows` host:
  - **Git Bash is now a first-class shell on Windows.** `resolveWindowsShell` already honored `$SHELL=…\bash.exe`, but the `buildWindowsBootstrap` dispatcher had no Git Bash builder — bash was falling through to the cmd.exe catch-all and exiting with code 126 as soon as it tried to interpret `/K`-style cmd args. New `GitBashBootstrapBuilder` emits a POSIX `cd; pwd; echo marker; … ; exec -l '<shell>' -i` script and is registered ahead of the cmd.exe fallback.
  - **ConPTY resize-reflow leak (all three shells).** Windows ConPTY keeps an internal screen buffer and re-emits the full buffer back through the PTY every time the terminal resizes. Our marker-handshake cleared xterm.js but never ConPTY's own buffer, so mid-session resizes replayed the pre-bootstrap `pwd` + marker lines into xterm as a "phantom header". Each bootstrap now appends a `CSI 2J CSI 3J CSI H` clear after the marker (`printf '\033[...]'` in Git Bash, `[Console]::Write([char]27 + '[2J' …)` in PowerShell, `cls` in cmd.exe — cmd has a scrollback-reflow caveat, see `known-issues.md`).
  - **Terminal resize-after-exit no longer spams red error logs.** `TerminalService.resize()` swallows the node-pty Windows-specific `"Cannot resize a pty that has already exited"` race and demotes the `!terminal` missing-id path from error to debug.
  - **Git status polling stops spamming WARN logs on non-git projects.** `GitPollingService.hasIndexChanged()` now detects `ENOENT` explicitly and logs once at debug; polling continues so a mid-session `git init` is still caught.
  ...
  - **Unit coverage for `WindowsTerminalBootstrap`** — new test file with 60 cases covering `canHandle` patterns, dispatch-chain precedence, script shape per builder (including the ConPTY clear), single-quote escaping, the loosened deny-list, and `normalizeWindowsCwd`.
  - **E2E path-sep bug** — `e2e/settings-logs.e2e.ts` hard-coded `.erfana/logs`; now uses `path.sep` so the assertion works on both Windows (`\`) and POSIX (`/`) hosts.
  - **ESLint ignore list** — `playwright-report/`, `test-results/`, and `coverage/` are now excluded so `npm run lint` doesn't try to parse minified Playwright trace bundles after an e2e run.
  - **11 pre-existing lint errors cleared** — unused test fixtures in `import-handlers.test.ts` / `transcription-handlers.test.ts`, two redundant inline `require()` calls, and four `no-useless-escape` warnings in `CameraService.test.ts`.
