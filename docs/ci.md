...
| `build` (Build) | `npx electron-vite build` + `node scripts/smoke/sqlite-worker-smoke.mjs` | yes | after the build, runs an always-on native-deps Node-ABI cross-check (better-sqlite3 prebuild load + FTS5 `MATCH`; SD-019/#19). The packaged mac/win smokes in `build_mac.yml` / `build_win.yml` are advisory (`continue-on-error`). Also the only gate on the **preload self-containment** guard — see below |
