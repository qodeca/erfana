...
| Secret Scan | `.github/workflows/secret-scan.yml` | active (**required check**) | push + PR + `merge_group` | `ubuntu-latest` | ~1 min | gitleaks (full git history) + trufflehog (verified secrets only). Version-pinned, SHA-256-checksum-verified binary downloads; no third-party actions |
...
|-----|---------|:---:|-------|
| `lint` (Lint) | `npm run lint` + `npm run lint:css` + `npm run design -- --check` | yes | Three **steps in one job**, deliberately. Branch protection matches the **job** `name:` (`Lint`), so a separate job called `Lint CSS` would be advisory only and would enforce nothing. `lint:css` is stylelint over `src/**/*.css` plus the `design/` cards' inline `<style>` (via `postcss-html`); `design -- --check` regenerates `design/` in memory and fails when a committed generated file is stale. `npm run lint` itself ends with `lint:shebang` (`scripts/check-shebangs.mjs`), which fails when a tracked `.js`/`.cjs`/`.mjs`/`.ts` (or `.jsx`/`.tsx`/`.cts`/`.mts`) file under `scripts/`, `src/` or `e2e/` starts with `#!`: a Windows CRLF checkout breaks the shebang for Vite and any test that imports the file fails to load, which only the advisory `Windows checks` job used to catch ([#151](https://github.com/qodeca/erfana/issues/151)). Run such a script as `node <file>`; the check's commented `ALLOWED` list is for a file that genuinely must be executed directly |
...
| `coverage` (Coverage) | three steps: `npx vitest --run --config vitest.main.ts --project main --coverage`, then the same with `vitest.preload.ts --project preload` and `vitest.renderer.ts --project renderer` | yes | enforces the coverage **floors** of all three vitest configs. Main: the per-file floors in `vitest.main.ts` `test.coverage.thresholds` — `scripts/fuses.js` (the #43/#55 packaging-integrity guards + Electron fuses, floor lines/statements 86, functions 88, branches 93; met at ~88) and the whisper trust-chain modules `verifyManifest` / `secureDownloader` / `zipArchive` / `tarArchive` (90% each) plus the #41 `modelId` registry (95%) and the #60 `src/main/utils/rendererCrashHandlers.ts` crash trail (90% each metric). Since `test` runs `test:ci` **without** coverage, these floors gate only here. Scoped to `--project main` (with `all: false`) so `scripts/fuses.js` reports a single deterministic row — see the design note below (issue #55, F4). Preload (#124): the project-wide floors in `vitest.preload.ts`. Renderer (#124): the per-file floors in `vitest.renderer.ts` for `usePreviewBounds`, `previewClip`, `previewDragFreeze` and `previewTabMove`, plus the project-wide floor. None of the preload or renderer floors depend on symlink cases, so they hold on the Linux runner. Added to the branch-protection required set on `main` |
| `build` (Build) | `npx electron-vite build` | yes | also the only gate on the **preload self-containment** guard — see below |
| `license` (License compliance) | `npm run check:headers` + `pipx run reuse lint` | yes | SPDX headers on all sources + REUSE conformance. `check-spdx-headers.mjs` covers `.ts .tsx .js .mjs .cjs .css .html` and skips `.xezar/`, whose Xezar kit files keep their own MIT licence (annotated in `REUSE.toml`, text in `LICENSES/MIT.txt`); a script that *emits* an SPDX header in a string literal must wrap it in `REUSE-IgnoreStart` / `REUSE-IgnoreEnd`, or `reuse lint` parses the literal as that file's own licensing and fails (see `scripts/design-sync.mjs`, `scripts/check-spdx-headers.mjs`) |
...
**Required status checks on `main`** (seven): `Lint`, `Typecheck`, `Unit tests`, `Build`, `License compliance`, `Coverage` (from `checks.yml`), and `Secret scan` (from `secret-scan.yml`). Note what `Lint` now blocks beyond ESLint: a raw hex colour, a bare `z-index`, a `border-radius` that is not `0` or the circle token, and a stale `design/` — a token violation or an unregenerated design system fails the merge, not just review. `npm audit signatures`, `Release readiness guards`, and `Windows checks` run on every push but are not required to merge. `Coverage` (issue #55, F4) enforces the per-file coverage floors that the required `test` job — which runs `test:ci` without `--coverage` — does not. These seven gate **pushes**, not just merges: `main` refuses any SHA whose seven checks have not already completed green (`GH006 … 7 of 7 required status checks have not succeeded`), which is why release-prep commits land on `develop` first — see [release.md § Branch protection](build/release.md#branch-protection-phase-i--done-2026-04-25). `develop` requires the same seven (strict mode), set up with the Xezar pipeline; administrators are **not** enforced there, so the develop-first push still works. All seven also run on `merge_group` events, so they report inside the `develop` merge queue – see [Merge queue](#merge-queue-develop).

**Why the `extraResources:` warning is edit-triggered** (issue #55, F4): the guard's contract is to warn when the `extraResources:` block is **edited**, so the change is a visible prompt to re-check `ALLOWED_EXTRA_RESOURCES_DESTS` and the binding test. It originally warned on the block's mere *presence* — and since `extraResources:` is permanently present (`tessdata`, `LICENSE`, `THIRD-PARTY-LICENSES.md`), the annotation fired on every push and could not signal an edit at all. It now extracts the block from the working file and from the push/PR/queue base commit (`github.event.before`, `pull_request.base.sha`, or `merge_group.base_sha`) and warns only when the two differ — hence `fetch-depth: 0` on the job's checkout. It stays **fail-safe**: when the base is unknowable (new branch, force-push, shallow history) or the config path is not the tracked repo file (the manual negative tests point it at a mutated copy), it falls back to warning rather than staying silent. This layer is advisory either way — the hard gate for `from`/`to` shape is the binding test in the required `Unit tests` job (`scripts/fuses.test.mjs`, "agrees with the repository electron-builder.yml as shipped"), plus the runtime L1/L2a guards in `scripts/fuses.js`.

**Why a separate `Coverage` job** (issue #55, F4): the per-file floors in `vitest.main.ts` fire only under `--coverage`, and the required `test` job runs `test:ci` without it — so before this job the `scripts/fuses.js` floor (and the whisper floors) were enforced only by a local `npm run test:cov`, never in required CI. The job runs the **main project only** (`--project main`): just the main suite loads `scripts/fuses.js` (via `scripts/fuses.test.mjs`), so the v8 report carries exactly one `fuses.js` row at its real coverage. Combined with `all: false` in `vitest.main.ts` (which suppresses the synthetic 0%-baseline row an untested-but-included file would otherwise emit), the threshold match is deterministic; a full-workspace coverage run could instead surface a second 0% `fuses.js` row and fail the gate spuriously. The preload and renderer floors (#124) run as two more **steps** in this job rather than as new jobs, because branch protection matches the `Coverage` check name; each step keeps its own `--project` flag (the auto-discovered `vitest.workspace.ts` would otherwise expand every run to all three projects) and writes to its own `coverage/<project>` directory.

**The local `npm run test:cov` carries the same `--project` flags** (issue #133). `scripts/test-cov.mjs` runs the three configs scoped as `--project main|preload|renderer`, matching the three CI steps, so each project's suite runs once instead of the whole workspace three times. It runs **all three passes before it exits**, so one project's missed floor cannot hide the other two's: a failing pass prints `FAILED: <project> coverage …`, and the run ends with `Coverage FAILED for project(s): <names>` naming every project that missed a floor, then exits non-zero (the vitest threshold lines above it name the file and axis). The per-area scripts `test:main`, `test:preload` and `test:renderer` carry the same `--project` flag for the same reason. One #133 defect remains open: `spawnSync(…, {stdio: 'inherit'})` child output still does not reach a redirected log on Windows, so there the vitest threshold lines may be absent from the log — the script's own summary line still names the failing project, and `npx vitest --run --config vitest.main.ts --project main --coverage` remains the way to see the full per-file detail.

**Test workers** (issue #171): every vitest run – CI, the local gate, agents and humans – caps its pool at half the cores, never fewer than two (`vitest.workers.ts`, read by `vitest.config.ts` and the three `vitest.<project>.ts` files, because vitest 3 sizes the pool from the root config only). On the 4-core `ubuntu-latest` and Windows runners that is 2 workers instead of vitest's default 3. Before the cap, `Unit tests` took about 2.5 minutes and `Coverage` about 4 (run of 2026-09-26), so even a proportional 1.5× slowdown stays far inside the 10- and 15-minute budgets. CI sets no override; `VITEST_MAX_WORKERS` (a count or a percentage) is the escape hatch if a job ever needs more.
...
**Design notes**:
- **`on: push:` + `merge_group:`** (not `pull_request`). Same-repo PRs already trigger a push event on their source branch; adding `pull_request` would double-run the same SHA. `merge_group` exists only so the required checks report inside the `develop` merge queue. The consequence for fork PRs (no checks at all, and the `github.event_name == 'pull_request'` half of every job guard never fires) is tracked in [#105](https://github.com/qodeca/erfana/issues/105).
- **No path filters**, ever. A required check that is skipped by a filter never reports, and a PR (or a merge-queue entry) waiting on it waits forever.
...
`develop` is meant to land PRs through a GitHub merge queue (#173). Strict required checks alone made every push to `develop` – each merge and each campaign-record push – put every open PR behind, forcing an update and a full CI re-run. A queue tests each PR against the latest `develop` instead, so an open PR no longer has to be up to date by hand.

How a PR lands once the queue is on:

1. The PR's own branch goes green on the seven required checks (from its `push` runs).
2. Adding it to the queue (**Merge when ready**) makes GitHub build a temporary `gh-readonly-queue/develop/pr-<n>-<sha>` branch holding `develop` plus the PR (plus any PRs ahead of it).
3. `checks.yml` and `secret-scan.yml` run on that ref as a `merge_group` event. Every required job carries only the `github.repository == 'qodeca/erfana'` guard, which always holds for a merge group, so all seven checks – `Lint`, `Typecheck`, `Unit tests`, `Build`, `Coverage`, `License compliance`, `Secret scan` – report under the same names branch protection expects. The advisory jobs run there too.
4. When all seven are green, GitHub fast-forwards `develop` to the tested commit; a red check drops the PR from the queue.

Notes:
- The concurrency groups key on `github.ref`, and every queue entry has its own ref, so a queue run never cancels a branch run or another entry.
- `release.yml` looks up a green `checks.yml` run by commit SHA. A `merge_group` run for the same SHA qualifies too; it ran the same `npm audit signatures` job on the same tree, so its `package-lock-digest` is identical.
- Direct pushes to `develop` (the leader's record pushes, the develop-first release step) are not affected: administrators are not enforced on `develop`.
- **The setting is not in the repo.** After the `merge_group` triggers are on `develop`, the owner enables **Require merge queue** in `develop`'s branch protection (Settings → Branches, or the equivalent ruleset). Turning it on before the triggers land would leave every queued PR waiting for checks that never start.
- `main` is unchanged: no merge queue, the same seven required checks, `enforce_admins` on, and releases still land by direct push after `develop` is green.
...
**The cost is real, though, and worth weighing against the flakiness.** #121 shipped `e2e/workspace-layout.e2e.ts` carrying a recent-projects test that had never run anywhere and was broken — it assumed an empty recent-projects list, but `userDataDir` is worker-scoped while the app fixtures are test-scoped, so every project an earlier test opened is still listed. It was found and fixed on 2026-09-15, the first time anyone ran the suite on Windows, a month after it merged. With the suite disabled, nothing catches a spec that is wrong on every platform.
