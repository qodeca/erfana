...
Run `npm run test` for current totals (~8,020 tests across 255 files as of v0.10.1 on macOS — 78 cases platform-gate on Windows (77 POSIX-only `pathSecurity.test.ts` + 1 macOS-only `LiteParseConverter.test.ts`), so Windows sees 7,942 pass / 78 skip and Linux sees 8,019 pass / 1 skip; invariant total stays 8,020. The Phase-2 UAT hardening merged in v0.9.3 added `WindowsTerminalBootstrap.test.ts` with 60 strategy-layer unit tests; v0.9.4 added Phase 4 whisper trust-chain tests and the D12 `WhisperModelManager.test.ts` rewrite; v0.9.6 added `scripts/fuses.test.mjs` with 9 cases covering the `afterPack` chmod helper for `node-pty` `spawn-helper`; v0.10.0 added `prompts/applyFooter.test.ts` (4 cases for the pure footer-composition function) + `prompts/mutation-templates.test.ts` (24 cases — invariant that every mutating registry id carries the `mutatesDocument` flag, that the footer is composed onto every rendered mutation prompt with a non-empty file reference, that no body contains the competing "return only / no commentary / no explanation" wording, plus a golden inline snapshot of the rendered `modify` prompt), and `components/DockLayout/terminalExpand.test.ts` (4 cases for terminal maximize over the editor); v0.10.1 added no new vitest tests (the sash-drag fix is covered end-to-end by `e2e/terminal-resize.e2e.ts` — see [E2E testing](./e2e-testing.md#test-files)).
