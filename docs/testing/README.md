...
Run `npm run test` for current totals. The workspace holds **562 test files** (under `src/`, `scripts/` and `tests/`, counted 2026-09-15). A full `npm run test:ci` reported **11,766 cases passing with none skipped** on a macOS host at v0.19.0, and **11,649 passing with 108 skipped** (11,757 collected) on a Windows host two commits earlier. Both were counted on 2026-09-04; re-run `npm run test:ci` for the live figure, since the case count moves with every commit while the file count is checkable with a glob. **The two host figures are not comparable and neither is wrong** – the gap is the skip set below, which is what a Windows host declines to run. At v0.19.0, **108 cases were skipped on a Windows host**, all of them in the `main` project: 77 in the POSIX-only `pathSecurity.test.ts`, 20 in `scripts/fuses.test.mjs` (the POSIX chmod and symlink cases), 5 in `projectConfinement.test.ts`, 5 across the file handlers, and 1 macOS-only `LiteParseConverter.test.ts` case. #124 added more win32 skips since then – `describe.skipIf` blocks in `BrowserLaunchService.integration.test.ts`, `previewFrames.integration.test.ts`, `previewNavigation.integration.test.ts` and `previewRequestKind.integration.test.ts`, and `it.skipIf` cases in `BrowserLaunchService.test.ts`, `previewFrameSources.test.ts` and `projectConfinement.test.ts`, all still in `main` – and the new total has not been counted on a Windows host. (`tarArchive.test.ts` gates its symlink case with an early `return` instead, so it reports as passing rather than skipped – which is why the coverage floors, not the test run, are what fail on Windows.) For the version-by-version test-addition history, see [`docs/CHANGELOG.md`](../CHANGELOG.md).
