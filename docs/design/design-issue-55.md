...
**Status**: shipped – issue #55 closed 2026-08-09. The L2a-1 guard designed below as `assertResourcesConfigDestsAllowlist` shipped under the name `assertResourcesDestNoRepoLeak` (`scripts/fuses.js`; §4 and §11 already use the shipped name, §3/§5.3/§8 keep the design-time one).
...
| **F8** — Windows L2a-2 advisory | no action | Already-approved F2 decision, tracked as technical-debt #15 (promote to fatal once a real Windows packed-tree baseline exists). |
