...
**Status**: shipped – issue #60 closed 2026-08-12 · approved at QG-4 on 2026-08-11 · **review_level**: full · **QG-4a lens review**: 31 findings folded in (see §10)
**Test-plan drift (2026-09-05)**: the "no double-log" assertion listed for `installGlobalErrorTrail.test.ts` in §3, §5 and §6 does not exist – per the §2.3 correction both the trail and the logger's own pair fire, so the shipped test asserts idempotence ("registers one listener per event type however often it is called", "logs one record per event after repeated installs"), not suppression.
**Issue**: #60 · **Tier**: n/a (bug/hardening) · **Phase**: 4 (Architecture)
**Reconciled with the shipped code on 2026-08-11**: §2.3 (panel copy, copy deck, global-trail claim), §3 (file-table split + four entries added during implementation, plus the recorded line-count deviations), §7 and §8.
**Second reconciliation pass, 2026-08-12** (documentation-drift audit — the code moved twice more after the first pass): §1 (entry-module failure class + the `Record` column qualified), §2.3 (three-arm panel focus contract, degraded-mode `aria-describedby`, corrected `logger.ts` citation), §2.6 (third export `registerWindowErrorSignals`), §2.7 (bounded, not merely `try/catch`-wrapped, restart path), §3 (eight unlisted shipped files added, counts 42 → 50, `ProjectTree.tsx` grew rather than shrank, contrast test reads both stylesheets, three line-count deviations re-measured), §5, §7 and §9. Passages marked *as shipped* or *corrected* override the design-time text above them.
...
| Renderer/utility process death, hang | main-process handlers — `registerAppCrashLogging()`, `registerWindowResponsiveness(win)` | OS-level blank/beachball (unchanged) | main log; `reason` + `exitCode` **for the two `*-process-gone` handlers only** (a child process adds `type`, plus `serviceName` / `name` when Electron supplies them), `windowId` on the hang edges |
| Entry-module or preload failure before React mounts *(as shipped)* | main-process `registerWindowErrorSignals(win)` | Empty page — no process death, no hang, and nothing recorded renderer-side | main log: `[crash] renderer-console-error` (`windowId`, `message`, `line`, `sourceId`) and `[crash] preload-error` (`windowId`, `preloadPath`, `error`) |
| Main-process crash | nothing in this issue | app exits | OS crash report |

*As shipped*, the entry-module row above was added during the QG-11a round (§2.6). It is a distinct failure class, not a variant of the two above it: nothing dies, nothing hangs, and no renderer-side handler exists yet, so the window simply shows an empty page. The `Record` column is therefore no longer uniform — "reason + exitCode" describes the two `*-process-gone` handlers and nothing else.
...
*As shipped*, the panel copy reads **"Project tree unavailable. The rest of Erfana still works."**, and a failed retry reads **"Project tree is still unavailable."** rather than repeating the reassurance; the action is a `Reload` button whose accessible name is `Reload project tree`. Two behaviours were added at implementation time and are load bearing: a deliberate focus contract (the rebuild would otherwise drop focus to `<body>`), and the boundary being **keyed by project** — `<PanelErrorBoundary key={projectPath ?? 'none'} …>` — because the error state survives re-renders, so without the key a tree that crashed on project A still reads "unavailable" after the user opens project B.

*As shipped*, that focus contract has **three arms**, not one, driven by a `getSnapshotBeforeUpdate` reading of where focus stood before the mutation phase (the only point at which it is still knowable):

1. **failed retry** — `reloadAttempts` grew and the fallback is still on screen ⇒ focus the Reload button;
2. **panel death with focus inside the panel** — including an async re-throw after a reload succeeded, where no attempt counter changes ⇒ focus the Reload button;
3. **successful Reload** — the fallback the user was standing on is gone ⇒ focus the recovered content wrapper (`.panel-error-content`, `tabIndex={-1}`).

And the fourth case is the deliberate non-action: a panel that throws while the user is in the editor **never** takes focus. That is what makes the "one announcement channel" trade-off honest — the announcement rides the focus move, so a panel dying out of view announces nothing and the copy waits for the user to arrive.
...
> **Corrected against the shipped code.** This section originally claimed the trail "supersedes" the renderer logger's own pair "without double-logging". It does not, and cannot from this module. `RendererLogger.installErrorHandlers()` (`src/renderer/src/utils/logger.ts` — *corrected*: the method is defined at `:267-286` and called from `initialize()` at `:145`; the `:143-160` citation above pointed at `initialize()`, not at the handlers) registers its own `error` / `unhandledrejection` listeners at `error` level, and **both fire**: one uncaught error produces a `fatal` line from the trail *and* an `error` line from the logger. Suppressing the logger's pair from the trail would require `stopImmediatePropagation()`, which would silently kill every `error` listener registered after this one — a worse defect than a duplicate log line. The duplicate is accepted and stated plainly in the module docblock; collapsing the two belongs in `logger.ts` and is recorded as a follow-up in §8. What the trail *does* guarantee, and what the tests assert, is idempotence (installing twice registers one listener per event type) and payload-shape parity with the boundary.
...
- Fallback container: `role="alertdialog"`, `aria-modal="true"`, `aria-labelledby={headingId}`, `tabIndex={-1}`, focused on mount. The description is `aria-describedby={messageId}` in the normal case; *as shipped*, degraded mode points at **both** degraded paragraphs — `aria-describedby={`${degradedId} ${logPathId}`}` — because the instruction ends on "Log files are in:", so a description that stopped there would leave the sentence hanging and the one thing the user still needs unspoken. There is **no** separate `role="alert"` block — one announcement, and the container carries a real accessible name (asserted with `toHaveAccessibleName`, not merely `toHaveFocus`).
...
**Main.** One module, `src/main/utils/rendererCrashHandlers.ts`. Designed with two exports; **three as shipped** — the third landed in the QG-11a round:
...
- `registerWindowErrorSignals(win)` — ***as shipped***. Per window, called from `createWindow` alongside the responsiveness registrar (`index.ts:186`). Observes `webContents`, not the window: `console-message` filtered to **`error` level only** (`info` / `warning` / `debug` are ordinary renderer chatter and would drown the trail, and in dev would echo Vite's own output into the support log) → `[crash] renderer-console-error`, and `preload-error` → `[crash] preload-error`. Closes the blind spot the other two leave: a bad import, a throw before `main.tsx` mounts or a preload that never finished produces no process death, no hang and no renderer-side record — the window just shows an empty page. Every renderer-supplied value is untrusted, so it is passed as **structured context** (never interpolated into the message), truncated at 1 000 characters with a `[truncated]` marker, and bounded in COUNT by a per-window fixed-window rate cap of **20 console-error records per 10 s**. The window opens on the first record and closes 10 s later, emitting one `[crash] renderer-console-error suppressed` line with the dropped count — only if anything was dropped, on an `unref`'d timer, and at `error` level so the gap shows wherever the records themselves would. `preload-error` is left uncapped: it fires at most once per load, and capping it could drop the one record that explains a blank window.

Log-only in all three cases: no auto-reload, no dialog, no relaunch (boot-loop safety).
...
*As shipped*, the call is **bounded, not merely wrapped**. `try/catch` covers a rejection but not the realistic failure after a crash: a main process that is itself unwell and never answers, leaving the user on a screen whose only working control has quietly stopped working. `closeProjectBestEffort()` therefore races the invocation (itself inside the async body, so a synchronous throw from a broken bridge is caught too) against a `CLOSE_PROJECT_TIMEOUT_MS` timer — **1 500 ms**, comfortably inside the 3 000 ms `RESTART_STALLED_MS` window, so even the worst case reaches the relaunch before the screen offers manual-quit guidance. Three outcomes, one behaviour: settle, reject or never settle, the relaunch runs unconditionally; the timer is cleared in a `finally` so it cannot outlive the screen.
...
**50 entries as shipped (22 create, 28 modify)** — the merge commit `69df015` reports 51 changed files; the 51st is this design document itself, which has never carried a row in its own table. The designed set was 38 (**19 create, 19 modify**) — the header previously read "17 create, 21 modify", which was a miscount of this same table, not a different set of files. Four modify entries were added while implementing and are marked *added during implementation* below. A further **eight** shipped files carried no row at all until a documentation-drift audit on 2026-08-12 found them; they are marked *added during implementation (recorded 2026-08-12)* and were the reason the header read 42. Two entries beyond the 36 agreed at seam-resolution are called out in §9: `ProjectPanel.tsx` (the mount point that must wrap the tree) and `src/preload/index.d.ts` (the type surface for the E2E flag).

| File (repo-relative) | Action | Description |
|---|---|---|
| `src/renderer/src/components/ProjectTree/ProjectTree.tsx` | modify | **Commit 1**: `useCallback` for `executePaste` / `handlePaste`, `handlePaste` added to the `:699` dep array. **Commit 3**: delete `enhancedFlattenedItems` (`:330-351`); destructure and use `findNode` / `findNodeWithRoot` at `:139`; repoint `:220`, `:476`, `:582`, `:666`, `:1143`, `:1391`; update deps at `:699`, `:1154`. *Corrected*: the net line count **grew**, 1 422 → 1 457 as shipped. The deleted `enhancedFlattenedItems` copy is outweighed by the `useCallback` wrapping from commit 1 and by the QG-11a expansion-seed fix. |
...
| `src/renderer/src/components/RootErrorBoundary/RootErrorBoundary.contrast.test.ts` | create | WCAG 1.4.3 AA ratios recomputed from `design-tokens.css`, modelled on `Dialog.contrast.test.ts`. *As shipped* it reads **both** crash stylesheets — `RootErrorBoundary.css` **and** `Panels/PanelErrorBoundary.css` — since both are the same containment set and one guard beats a second copy of the machinery. It also carries the panel sheet's **`.panel-error` prefix allowlist** (`scopes every PanelErrorBoundary.css selector to .panel-error`): there is no `PanelErrorBoundary.css.test.ts`, so this is where that guard lives. Cross-checks run in both directions, so neither a new coloured element nor a new background can be added unmeasured. |
...
| `src/renderer/src/utils/installGlobalErrorTrail.test.ts` | create | Both event types, payload shape, idempotence. *As shipped* there is no "no double-log" test – see the status note at the top and §2.3. |
...
| `src/main/utils/rendererCrashHandlers.ts` | create | `registerAppCrashLogging()` (app-level `render-process-gone` + `child-process-gone`, `reason` + `exitCode`) and `registerWindowResponsiveness(win)` (`unresponsive` / `responsive`). *As shipped*, a **third** export: `registerWindowErrorSignals(win)` — per-window `console-message` (error level only) + `preload-error`, structured untrusted context, 1 000-char truncation, 20-records-per-10 s cap with a dropped-count summary (§2.6). |
| `src/main/utils/rendererCrashHandlers.test.ts` | create | All exports: registration, payload fields, single registration, no side effects. *As shipped* it also covers the third export — level filtering, truncation, the rate cap and its summary line, per-window counters, and the uncapped `preload-error`. |
| `src/main/index.ts` | modify | `backgroundColor` in the `BrowserWindow` options; `registerAppCrashLogging()` in `whenReady`; `registerWindowResponsiveness(mainWindow)` and — *as shipped* — `registerWindowErrorSignals(mainWindow)` in `createWindow`; unpackaged-only `additionalArguments` for the E2E flag. |
...
| `src/renderer/src/components/Panels/PanelErrorBoundary.css` | create | *Added during implementation (recorded 2026-08-12).* The panel fallback's stylesheet. Every top-level selector is prefixed `.panel-error` — the same shared-entry leak constraint `RootErrorBoundary.css` is under (§2.4) — and that prefix is enforced from `RootErrorBoundary.contrast.test.ts`, not from a stylesheet test of its own. Also carries `.panel-error-content`, the flex passthrough that wraps healthy children and takes focus after a successful Reload. |
| `src/renderer/src/components/ProjectTree/ProjectTree.expansion.test.tsx` | create | *Added during implementation (recorded 2026-08-12).* Covers the QG-11a expansion-seed fix in `ProjectTree.tsx` — the one QG-11a change outside the containment layers. |
| `src/preload/screenshotOverlay.test.ts` | create | *Added during implementation (recorded 2026-08-12).* Covers the overlay preload's one-way log forward below. |
| `src/preload/screenshotOverlay.ts` | modify | *Added during implementation (recorded 2026-08-12).* Exposes a one-way `log` forward on `overlayApi`, reusing the main preload's existing `logging:log` channel (validated main-side by `LogEntrySchema`, so no new IPC surface). The overlay renderer never gets `window.api`, so a failure in the one window with **no** recovery UI previously left no evidence at all. |
| `src/renderer/src/utils/logger.ts` | modify | *Added during implementation (recorded 2026-08-12).* Load bearing for §2.3, which discusses this module's `installErrorHandlers()` but never listed it as touched. Adds `resolveLogSink()` — `window.api` first, then the overlay bridge above — and a console fallback when neither exists, so a renderer with no bridge at all still leaves a record that main mirrors through the `console-message` trail (§2.6). |
| `src/renderer/src/utils/logger.test.ts` | modify | *Added during implementation (recorded 2026-08-12).* Sink-resolution and fallback coverage for the change above. |
...
|---|---|---|
| `src/renderer/src/hooks/useDragDropTree.test.ts` | 1 127 | Carries the 200k reproduction, the O(n) invariant scan, the ~20 000-level depth chain and the index/staleness suites against one hook. Splitting separates the reproduction from the parity assertions that give it meaning. |
| `src/renderer/src/components/RootErrorBoundary/RootErrorFallback.test.tsx` | 613 | Per-describe `vi.useFakeTimers` with a substrate-specific `shouldAdvanceTime` recipe (documented in its docblock). The repo's split policy keeps per-`describe` fake-timer suites in one file and splits only when mocks hoist to module scope — see `docs/windows/contributing.md` §"Test-file split policy". |
| `src/main/utils/rendererCrashHandlers.test.ts` | 651 | *Recorded 2026-08-12.* Created by this change and over the guideline from the start. One fake-window harness (`createFakeWindow`, `emitWebContents`, `burst`) serves all three registrars; splitting it would either duplicate the harness or leave the rate-cap suite reaching into a sibling file for it. This file also carries the enforced per-file coverage floor in `vitest.main.ts`, so its subject is deliberately one module. |
...
| `installGlobalErrorTrail` | `error` and `unhandledrejection` both reach `logger.fatal` with the boundary payload shape; installing twice registers once (one record per event after repeated installs). *As shipped* no row asserts that a boundary-caught error is not double-logged – §2.3 records that both the trail and the logger's pair fire. |
...
| Main handlers | `registerAppCrashLogging()` registers `render-process-gone` + `child-process-gone` on `app` exactly once and logs `details.reason` + `details.exitCode` (no `killed` field); `registerWindowResponsiveness(win)` registers `unresponsive` / `responsive`. *As shipped*, a third registrar: `registerWindowErrorSignals(win)` records only `error`-level console messages, truncates untrusted strings at 1 000 chars, caps at 20 records per window per 10 s and then emits one dropped-count summary, counts each window separately, and leaves `preload-error` uncapped. None of the three reloads, destroys, or relaunches. |
...
| Duplicate fatal lines in dev (React re-throws to `window` after `componentDidCatch`) | high (dev only) | low | Accepted and documented in §1; the trail is idempotent (one listener per event type). *Corrected*: it does not suppress the logger's own `error`-level record – both fire, per §2.3. `componentDidCatch` is **not** StrictMode-double-invoked; the `useMemo` is, which is why the timing log moved to an effect. |
...
Files: 50 as shipped (22 create, 28 modify) — see §3. The QG-4a findings roughly doubled the original 13–16 h estimate, almost entirely in the containment and accessibility layers.
...
Boundary classes follow `EditorErrorBoundary` / `GitErrorBoundary`; `PanelErrorBoundary` sits beside its consumer, as `GitErrorBoundary` does. Co-located global CSS with tokens and `border-radius: 0`; contrast test modelled on `Dialog.contrast.test.ts`; `aria-disabled`-over-`disabled` follows this repo's own `Dialog/CLAUDE.md`; TEST_IDS centralised with the count-test convention; `(window as any).api` mocking, never `vi.stubGlobal('window', …)`; source-text invariant tests follow `main.import-isolation.test.ts`; E2E flag reuses the shipped `additionalArguments` + `process.argv` overlay-token mechanism; main-process handlers extracted to `src/main/utils/` rather than growing `index.ts`, consistent with the extract-don't-extend rule for the oversized `ProjectTree.tsx`.

*Corrected against the shipped tree (2026-08-12).* Both sizes cited here were design-time figures and both are now wrong, in the same direction: `src/main/index.ts` is **614 lines** (was 428) and `ProjectTree.tsx` is **1 457** (was 1 422). So the extraction kept the crash handlers out of `index.ts` but did not stop it growing — the quit-confirmation timeout and the window-scope registrations landed there — and the claim that this plan **shrinks** `ProjectTree.tsx` is false: it grew by 35 lines (§3). Neither is a scope failure; the extract pass over `ProjectTree.tsx` stays owned by #149/#150, and the pattern-alignment argument (extract, don't extend) holds regardless of the absolute numbers.
