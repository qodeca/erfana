...
> **SUPERSEDED for model-id parsing and window sizing — [#41](https://github.com/qodeca/erfana/issues/41).** The Opus-only capability registry described in §2 ("200k-vs-1M detection"), the friendly-name rule in §2, the `ClaudeWindowDetector.detect` step of the §3 pipeline (its "registry → used>200k → settings.json" ordering is not the shipped chain), the `ClaudeWindowDetector.ts` / `friendlyModelName.ts` entries in §4, the "Model-capability registry, design v3" bullet in §10 and the **sticky-window sub-bullet (b) of §10.1's "Post-compaction display"** are all superseded by [`41-model-capability-registry.md`](41-model-capability-registry.md). Where the two documents conflict, that one is authoritative. In particular: **Opus 4.6 runs at 200k** at the Claude Code layer (this document says 1M), **Sonnet 4.6 is 200k** without usage credits, Sonnet 5 / Fable 5 / Mythos are 1M, unrecognised ids from a known family now extrapolate within bounds **per major generation** (a known major inherits that major's newest window, not the family's) instead of always defaulting to 200k, and a registry-derived 1M is provisional (marked `(inferred)`, never latched). The §10.1 sticky bit is therefore **corroborated-only**: it still latches a 1M observed in usage, selected via `/model …[1m]`, carried as a `[1m]` marker on the id, or configured in `settings.json`, but a 1M resting on the capability table alone is provisional and is never latched. Everything else in this document — placement, IPC, transcript location, process detection, security, accessibility — still stands.
...
- `src/main/services/claudeStatus/ClaudeWindowDetector.ts` (create, ~150) — model-capability registry (`modelNativelySupportsExtended`: Opus 4.6+ auto-1M) → `used>200k` → settings.json `[1m]` → default 200k. **SUPERSEDED by [#41](41-model-capability-registry.md) §8:** `modelNativelySupportsExtended` was **deleted**; the capability registry moved to the new `claudeStatus/modelId.ts` behind `windowForModelId`, which the detector calls directly. The rule chain gained an explicit-standard-selection rule (R0') and an explicit-`[1m]`-in-the-id rule (R1m). A spawn-env cap (R_ENV) was designed and then **withdrawn before merge** — see #41 §5.2 and §13(h).
- `src/main/services/claudeStatus/friendlyModelName.ts` (create, ~70) — override table + generic derivation + raw fallback. **SUPERSEDED by [#41](41-model-capability-registry.md) §6:** renders from the shared `parseModelId`; the override table and the local regex are deleted.
