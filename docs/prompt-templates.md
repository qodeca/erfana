...
| `targetPanel` | enum | ❌ | Target panel: `"terminal"` (default: `"terminal"`) |
...
Copilot panel removed; prompts target the Terminal panel.
...
**Default:** If `targetPanel` omitted, defaults to `"terminal"`
...
**Use Case:** Automatically run terminal commands generated by prompts
...
export async function executePromptTemplate(
  ...
  return await openPanelAndSendContent({
    panel: config.targetPanel || 'terminal',
    location: 'right',
