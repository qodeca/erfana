<!-- SPDX-License-Identifier: GPL-3.0-only -->
# Export to PDF or Word

Open Markdown in Split or Preview view, then select **Export to PDF** or **Export to Word**.

**Before you start:** open Markdown in **Split** or **Preview**; export is unavailable in **Editor Only**.

## Export Markdown

1. Select **Export to PDF** or **Export to Word** on the toolbar.
2. Choose a destination in the system save dialog.
3. Open the resulting document.

![Screenshot of the Markdown toolbar with Export to PDF and Export to Word buttons.](../images/export/toolbar-export-buttons.png)

> [!NOTE]
> Windows uses the same controls but a different save dialog.

## What happens next / If something goes wrong

HTML preview has its own PDF export. See [troubleshooting](../reference/troubleshooting.md) if export cannot start.

## Related

- [Export reference](../reference/export.md)
