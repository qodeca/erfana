...
| 9 | `graph/sqliteVersion.test.ts` | 60 | **M32.** `sqliteVersionAtLeast` parses to `X*1e6 + Y*1e3 + Z` and **rejects** `'3.4.0'`, `'3.9.0'`, `'3.34.9'`, `'3.36.99'`, **`'3.37.0'`, `'3.37.99'`** — every one of which passes a naive lexical `>=`, which is what revision 2 specified. Accepts `'3.38.0'`, `'3.53.3'`. Floor is `GRAPH_MIN_SQLITE = 3_038_000` because the JSON1 built-ins `json_each(:ids)` needs (`GRAPH_SEARCH_HYDRATE_SQL` + the delete sweep) became unconditional only in 3.38.0 ([17]); this subsumes both `STRICT`'s 3.37.0 and `RETURNING`'s 3.35.0. |
