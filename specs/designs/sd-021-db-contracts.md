...
**M4 — do not make correctness depend on the token.** `generation` persists only in `graph_meta`, which the rebuild **drops**. On `REBUILD('corruption')` the unreadable page may *be* `graph_meta`'s, and on `APPLY_FRESH` no prior value exists — so a counter's next value is undefined on exactly the two paths that matter, and a restart-at-1 could re-issue a value the reader already holds at the moment the tables were dropped. Since C3 keeps the same handle and no PRAGMA detects the swap, there would be no other signal, and the reader could keep executing statements prepared against dropped tables.
