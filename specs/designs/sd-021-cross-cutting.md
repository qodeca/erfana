...
| 11 | **FR-005 / NFR-004: zero SQL interpolation.** Three layers (§9.6). | `eslint.config.mjs`, `GraphSearchService.ts` | **#26** (sanitiser) / #23 (lint rule) |
...
1. **Envelope, once per response.** `GraphMcpToolResultSchema.untrustedContentNotice` is a required, non-empty constant emitted as the **first** content block, exported as `MCP.UNTRUSTED_NOTICE`:
   ...
   It is emitted once per response, not per result, so it cannot be diluted by a large result set.
2. **Control-character stripping.** Every string field (`snippet`, `heading`, `filePath`) has C0 (`U+0000`–`U+001F`) except `\t`/`\n`, and C1 (`U+0080`–`U+009F`), removed before serialisation. This kills ANSI escape sequences and the sentinel characters §6.5 uses internally — which must never reach an external consumer.
3. **Byte caps.** Per result `MCP.MAX_RESULT_BYTES` (8 KB) and per response `MCP.MAX_RESPONSE_BYTES` (64 KB), measured after serialisation. On truncation `truncated: true` is set so the model is told the view is partial rather than silently receiving a clipped corpus.
...
**Owner: #26**, matching §2.3 and §4.2 (revision 2's row 11 said #23, contradicting both). **Named test, inherited by #26:** `ftsQueryBuilder.property.test.ts` — no input of length ≤ `MAX_QUERY_LENGTH` produces an FTS5 syntax error, over a corpus of `"`, `""`, `*`, `^`, `-`, `:`, `NEAR(`, `{`, `}`, `(`, `)`, lone surrogates, combining marks, and a 4096-character passage. Layer (c), the ESLint sink rule, bans `TemplateLiteral` and `BinaryExpression[operator='+']` inside `.prepare()`, `.exec()`, `.run()`, `.all()`, `.get()`, `.pragma()`, `queryAll()`, `queryGet()`, `querySearchPage()` under `src/main/services/graph/**` and `workers/graph/**` (precedent `eslint.config.mjs:107-127`); owner #23.
