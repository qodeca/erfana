...
The system shall register an `erfana_graph_search` tool with parameters: query (string), k (number, optional), filters (object, optional). Tool registration shall use the SDK v2 `registerTool` API with `title`, `description`, and Zod input schema.
