...
describe('sqliteVersionAtLeast', () => {
  it('uses the 3.37.0 floor, which STRICT tables require', () => {
    expect(GRAPH_MIN_SQLITE).toBe(3_037_000)
  })

  it.each(['3.4.0', '3.9.0', '3.34.9', '3.36.99'])('rejects %s', (version) => {
  ...
  it.each(['3.4.0', '3.9.0'])('rejects %s, which a lexical compare wrongly passes', (version) => {
    expect(version >= '3.37.0').toBe(true)
    expect(sqliteVersionAtLeast(version, GRAPH_MIN_SQLITE)).toBe(false)
  })

  it.each(['3.37.0', '3.37.2', '3.45.1', '3.53.3', '4.0.0'])('accepts %s', (version) => {
