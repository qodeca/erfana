...
import { ConverterRegistry, createConverterRegistry } from './ConverterRegistry'
import type { ValidationResult, ImportResult, IConverter } from './types'
...
export class ImportService {
  constructor(private registry: ConverterRegistry = createConverterRegistry()) {}
   ...
   */
  async importFile(filePath: string, projectPath: string): Promise<ImportResult> {
    const ext = getExtension(filePath)
    let converter = this.registry.getConverter(ext)
    ...
    // Convert content
    const conversion = await converter.convert(filePath)
