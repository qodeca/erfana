...
`OverlayGuardService` (`../../../services/preview/OverlayGuardService.ts`) is the SINGLE owner of preview show/hide and the ONLY renderer module allowed to call `api.preview.setVisibility` (ESLint-enforced). It computes visibility PER live preview (the rule is stated once, under "Multiple live previews" below). Overlays register via `useOccluder(kind, active)` (`../../../hooks/useOccluder.ts`) – kinds `dialog`/`settings`/`toast`/`menu`/`overlay`/`drag`. Opening a dialog, settings, a menu (the badge popover), a full-screen overlay, or switching to another tab all hide the view. Never call `setVisibility` or manipulate the view from a component.
...
**The window-edge resize hold is the one sanctioned bypass of the single-hider rule (#124).** While a window edge is dragged, main hides that window's shown views itself (`previewResizeHold.ts` → `visibility.hold()`) and sends no `visibilityApplied`, so the guard keeps believing "visible". A `setVisibility` sent meanwhile is recorded by main, not applied; the hold ends through main's one `release()`, which applies the latest wish and reports it, so a dialog opened mid-hold stays covered. The renderer mirrors the hold in `usePreviewStore`'s `resizeHeld` for the still picture only – the guard never reads it. A **splitter** drag is not an exception: `previewDragFreeze.ts` (mounted once by `useSplitterDragFreeze`) registers the `drag` occluder, holds pointer moves until main confirms the hide, and on release waits two frames and pushes settled bounds before the occluder goes.

**The hide must be SYNCHRONOUS, main-side.** `PreviewLiveView.setVisibility(false)` hands straight to its visibility part (`previewLiveVisibility.ts`), which calls `view.setVisible(false)` in the same tick, and starts NO capture at all — still frames are taken while the view is DRAWN (`captureWhileVisible`, off `did-finish-load`), never on the way out. `PreviewViewService.setVisibility` is a pass-through to it for the same reason: there is nothing there to await. A native view takes pointer input over its own rectangle whatever the DOM says, so any wait before the hide leaves an overlay drawn on screen that cannot be clicked. Two sibling tests in `src/main/services/preview/PreviewViewService.test.ts` pin both halves: the hide asserts `expect(capture).not.toHaveBeenCalled()`, and the next one asserts the frame IS captured once the page is ready, while the view is drawn. **Never put I/O in front of `setVisible(false)`.** The ONE legitimate wait is eviction, which hides, then awaits `whenCaptureSettled()` before destroying the `webContents` — otherwise a suspended panel wakes with no picture. **"Never on the way out" is about STARTING a capture, not finishing one.** A capture started while drawn can still settle after the hide and may store its result when the panel has no frame yet (on macOS the first `capturePage` at `'ready'` is always empty, so the 250 ms retry is the only one that produces pixels). The `shouldKeep` veto applies wherever a frame already exists.

**The permission band is a second input to the guard, and it is per panel.** `usePreviewChromeGate` publishes a reason into `stores/usePreviewChromeGateStore.ts`; the guard reads it as the `gate` term of the visibility rule and reports `chrome-unconfirmed` / `chrome-too-short`. The same term also carries `collapsed`, from `stores/usePreviewCollapsedStore.ts`: the bounds hook raises it when every clipping ancestor leaves none of the placeholder visible, and the permission band's reason wins when both are set. It is deliberately NOT the occluder store: that one is global, so it would blank the other preview in a split view for a reason belonging to one panel.
...
The rule is per panel and has three terms: `visible = (panel is the active tab) && !occluded && gate === null`, where `gate` is the permission band's reason or `collapsed`. It yields at most one `true` only because dockview drag-and-drop is disabled (`EditorAreaSplitPanel.tsx`), which nothing enforces – so `recompute` logs in development if two panels ever compute visible at once. If you re-enable DnD, fix the guard first.
...
The `ResizeObserver` is not a dependable second chance either: dockview re-parents an `always`-rendered panel rather than resizing it in place, so the 0x0 -> laid-out transition need not produce a resize callback at all. `usePreviewBounds` runs a **per-frame measure loop** while `isVisible && isLive`, sharing one compare-and-send with the observer and the window-resize listener: a rect goes out only when it differs from the last one measured (the first measure of each run always sends), so a still panel sends nothing and a moving one at most one message per frame. It also catches the moves the observer cannot see – a panel that slides without resizing, and dockview's layout landing a frame after the window `resize` event (C1). `isLive` is the store load state leaving `'idle'`, which main emits AFTER `registry.install` – the earliest point a rect is not thrown away, which is why the panel reads the store BEFORE calling the hook.

**Clipped away is not the same as 0x0 (C2).** A collapsed editor keeps dockview's minimum width but sits inside a zero-wide `overflow: hidden` ancestor, so the placeholder still measures a real rect. The same measurement intersects it with every clipping ancestor (`previewClip.ts`: `overflow` other than `visible`, or paint containment); an empty result is never sent, and once a rect has gone out it raises the panel's `collapsed` flag, which the guard reads to hide the page. On return: one forced push, then the flag clears, then the guard shows.

**Every dropped update leaves a log line.** The hook reports each place it throws an update away through one rate-capped reporter per mount (`src/shared/dropReporter.ts`), with fixed reason ids in `PREVIEW_BOUNDS_DROP_REASON`: `disabled`, `no-placeholder`, `degenerate-rect` and `clipped-away` (R7, once per change of the measured rect). Main's side uses the same line shape and names its reasons in `src/main/services/preview/previewBoundsDropLog.ts` (`SET_BOUNDS_DROP_REASON`, `NO_VIEW_DROP_REASON`, `LIVE_BOUNDS_DROP_REASON`, `RESIZE_HOLD_DROP_REASON`), so one log search for a reason id finds both halves. Drop lines name the panel by `stablePathDigest`, never by its readable id.
...
Every `html-preview-*` e2e spec drives the preview through `HtmlPreviewPage` (`e2e/pages/html-preview.page.ts`, catalogued in [docs/testing/e2e-testing.md](../../../../../../docs/testing/e2e-testing.md)), never a private `open()` / `eval()` copy, and finds a placeholder through its owning panel's `aria-label` (`HTML preview of <basename>`, plus ` – press Enter …` while the page is keyboard-enterable).
...
- **Forwarded accelerators.** The native view swallows renderer keys, so main forwards exactly `f`/`s`/`w`/`Escape` plus `back`/`forward` (#124) via `preview:forwardedShortcut`; `usePreviewFindShortcuts` routes them to panel actions. Back and Forward match the physical key through `src/shared/previewNavKeys.ts` (Cmd+[ / Cmd+] on macOS, Alt+arrows elsewhere), so `accel` is `false` for both and they route on `key` alone. With focus in the panel's own chrome the same keys are caught by the panel root's `onKeyDown` (`usePreviewNavigation`) – not a `window` listener, so the active-panel gate holds by construction. Forwarded Escape must run the provider's `clearHighlights()` + restore focus (matching `SearchBar.handleClose`), not just flip the store flag. **Zoom keys are deliberately NOT forwarded** — the View menu owns them (see below). `PREVIEW_FORWARDED_SHORTCUTS` and `PreviewForwardedShortcutSchema` restate the same vocabulary in two layers and are pinned equal by a test in `previewInputForward.test.ts`.
...
- **Keyboard entry into the page (#124, QG-8 U1).** The native view is outside the renderer's tab order, so the placeholder is the way in: `usePreviewPageEntry` makes it `tabIndex 0` / `role="button"` only while the tab is ACTIVE (followed through `onDidActiveChange` – dockview keeps inactive panels mounted) and a page is live and drawn; Enter or Space calls the sender-gated `preview:focusPage`. Escape inside the page is the way back (focus returns to the band chip) – and it works only because main hands NATIVE focus to the host window before forwarding the key (`src/main/services/preview/previewHostFocus.ts`, also for Find and Close): `chipRef.focus()` alone is a DOM focus, which cannot take the keyboard back from a `WebContentsView`. Focus never moves on a load or a mount.
- **Tab identity is `params.filePath`, never the panel id.** A same-tab move keeps the panel id minted for the first page and writes the new page into `params.filePath` from `preview:pageChanged` (`usePreviewEvents`); title, tooltip, close label, lookups and crash recovery all follow it. `previewTabMove.ts` reads a panel's page from params for the same reason – never parse a path back out of an id (root CLAUDE.md).
- **What a move to another document resets.** `usePreviewStore.resetPage` drops the blocked hosts and their truncation flag, the still frame and the dropped-candidate count; a `#section` step resets nothing. Failures are deliberately NOT reset: the badge mirrors main's failure snapshot, which can arrive for the new page before `pageChanged` (RS2-3). A move to another document also closes the find bar and collapses the host list (`onLeavePage`).
- **One link router, one creator.** `EditorAreaSplitPanel` mounts it (`mountPreviewLinkRouter`, with the unsaved-changes prompt and `closePanel`) and disposes it on unmount; `getPreviewLinkRouter()` only reads it and returns `null` until then, and the panel's Back then stays `aria-disabled`. Never create a router elsewhere – one made without the prompt refuses every move that needs it.
- **The preview toolbar.** `.erf-band` (`components/PreviewChromeBand.tsx`) is always-DOM and sits ABOVE `.html-preview-page-area` as a flow sibling in a flex column, never absolute, and its height is never restated as a constant in `usePreviewBounds.ts`. Layout subtracts it: the placeholder's own box already excludes the bar at whatever height it happens to be, which is what lets it grow (the permission band opening its list) with no code change. Keep it that way – an absolute bar means two numbers only a comment keeps in step. The find bar keeps `SEARCH_BAR_INSET_PX` because it is still an overlay *inside* the page area. It is styled as a toolbar, matching `.markdown-toolbar` – 1px `var(--color-border-default)` under it, carrying Back and the link-mode toggle (`PreviewNavControls`) then Find leading, the permission chip, and the trailing tools Open in default browser then Export to PDF (`PreviewToolbarTools`), and no provenance label (decided; see [docs/security.md](../../../../../../docs/security.md) residual risk 8). What is load-bearing is structural: the bar renders unconditionally (never gated on something being blocked), it never scrolls with the page, and it stays a flow sibling above the page area – that is what keeps Erfana's own security question ("Approve this host?") in a region the page provably cannot paint, given that a toast leaves an untrusted page on screen while the question is asked. The panel root also holds a polite live region (`.erf-band__announce`, `data-testid="preview-move-announcement"`) saying what a landed move did; `usePreviewMoveAnnouncer` writes it at least one frame after `pageChanged`, so the text is not created with the region.
...
The listeners are siblings of `did-finish-load` in `previewViewLifecycle.ts`, **outside `pipeline.schedule()`** (`previewLivePipeline.ts`, wired in `previewLiveWiring.ts`) on purpose: that pipeline is rate-limited and drops events during a save burst, which would strand the backdrop in the chrome phase.
...
2. That growth moves the placeholder, so the bounds loop pushes with `{ ack: true }`.
3. `confirmRepaint` (`src/main/services/preview/previewLiveBounds.ts`) answers from isolated world 997 (a page cannot shadow `rAF`).
