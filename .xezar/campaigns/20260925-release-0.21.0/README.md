...
## Owner items
- **#186 S-5 follow-up issue (nit) – being fixed inside #191 round 1; issue not needed if it lands:** gate-parallel.mjs:67 – if all three guarded command names change together, the ordering guards turn off silently. Leader proposes filing it as an issue (title: "gate-parallel: refuse when no guarded command name is found"). Needs your word to create it.
